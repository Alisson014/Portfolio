...
export default function SkillsDeleteForm( { setIsUpdating, data } : SkillsAddDeleteType ){
    ...
    return(
        <div className="w-full bg-black border border-gray-700 rounded-lg p-4 appearAnimation">
            ...
            <div className="flex flex-col items-center gap-2 mt-4">
                {
                    data.map( d => (
                        ...
                        <div className="flex items-center gap-3">
                            <Image src={`${d?.icon ? data[0]?.icon : ''}`} alt="Skill image" width={30} height={30} />
                            <p>{d.name}</p>
                        </div>
            ...
            <ModalDeleteSkill isVisible={isVisible} setIsVisible={setIsVisible} ids={items} setItems={setItems} setIsUpdating={setIsUpdating} />
        </div>
    );
}